...
function renderGalleryNode(node, options = {}) {
  addCreateDocumentOption(options);
  const document = options.createDocument();
  const validImages = node.images.filter(isValidImage$1);
...
function parseCarouselNode(CarouselNode) {
  return {
    figure: nodeElem => {
      ...
      return {
        conversion(domNode) {
          ...
          payload.items = itemElements.map(readCarouselItemFromElement).filter(Boolean);
          payload.caption = readCaptionFromElement(domNode);
...
function isValidImage(item) {
  return item.type === 'image' && item.fileName && item.src && item.width && item.height;
}
function isValidVideo(item) {
  return item.type === 'video' && item.src;
}
function renderCarouselNode(node, options = {}) {
  addCreateDocumentOption(options);
  const document = options.createDocument();
  ...
  validItems.forEach(item => {
    ...
    if (item.type === 'image') {
      ...
      if (defaultMaxWidth && item.width > defaultMaxWidth && isLocalContentImage(item.src, options.siteUrl) && canTransformImage && canTransformImage(item.src)) {
        const {
          width,
          height
        } = getResizedImageDimensions(item, {
          width: defaultMaxWidth
        });
        img.setAttribute('width', width);
        img.setAttribute('height', height);
...
/* eslint-disable ghost/filenames/match-exported-class */
class CarouselNode extends generateDecoratorNode({
  nodeType: 'carousel',
  properties: [{
    name: 'items',
    default: []
...
const $createCarouselNode = dataset => {
  return new CarouselNode(dataset);
};
function $isCarouselNode(node) {
